...
/**
 * Class representing the scanner of an interpret. Main task of this class is
 * reading the input source program and parsing data into tokens.
 */
class Lex {
 private:
  /**
  ...
  enum lexState {
    ...
    COMMENT_LINE,         // //
    COMMENT_BLOCK_START,  // /*
    COMMENT_BLOCK_STAR,   // * in the block comment - possible end of block
                          // comment
 ...
 public:
  ...
  /**
   * Main public method. If there is a token on stack (from previous ungetToken
   * method calling), that token is returned. Otherwise, the new token is read
   * from stream
   * @return
   */
  Token& getToken();

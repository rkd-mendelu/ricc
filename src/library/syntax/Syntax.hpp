...
class Syntax {
 public:
  ...
  static const enum Operations precedence_table[SIZE_OF_PRECEDENCE_TABLE]
                                               [SIZE_OF_PRECEDENCE_TABLE];
  ...
  static constexpr const size_t numberOfLongRules =
      sizeof(Syntax::longRules) / Syntax::sizeOfLongRule;

  static constexpr const size_t sizeOfShortRule = sizeof(Syntax::shortRules[0]);
  static constexpr const size_t numberOfShortRules =
      sizeof(Syntax::shortRules) / Syntax::sizeOfShortRule;
 ...
 private:
  ...
  void putShiftToken(std::stack<std::reference_wrapper<Token>>& tokenStack,
                     Token& terminal);

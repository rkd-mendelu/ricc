...
Syntax::Syntax(std::istream& stream) : _lex(stream) { DEBUG(""); }
...
Interpret::Interpret& Syntax::getIntepreter() { return this->_interpret; }
...
const enum Syntax::Operations Syntax::precedence_table
    [Syntax::SIZE_OF_PRECEDENCE_TABLE][Syntax::SIZE_OF_PRECEDENCE_TABLE] = {
        /*            ==   >   <   >=  <=  !=  +   -   *   /   )   (   $   id
           &&  ||  li  !   =   */
...
Token& Syntax::getFirstTerminalFromTop(
  ...
  while (!s.empty()) {
    if (s.top().get().isTerminal()) return std::ref(s.top().get());
    s.pop();
  }
  return std::ref(ImplicitToken);
}

void Syntax::putShiftToken(
    std::stack<std::reference_wrapper<Token>>& tokenStack, Token& terminal) {
...
int Syntax::reduceStack(std::stack<std::reference_wrapper<Token>>& tokenStack,
                        std::reference_wrapper<Token>& inputToken) {
  ...
  do {
    if (counter > 3) {
      DEBUG("Trying to reduce stack more than expected"
            << "3<" << counter);
      // longest rule is 3
      DEBUG(" -> 1");
      return 1;
    ...
    if (counter == 1 && token.get().isRValue() &&
      ...
      if (token.get().isLiteral()) {
        ...
        if (item.get() == nullptr) {
          if (this->_semanticsCheck) return SEMANTICS_ERROR;
        } else if (item->isVar()) {
  ...
  if (counter == 3) {
    for (size_t i = 0; i < numberOfLongRules; ++i) {
      if (longRuleIndex[i]) {
        DEBUG("MATCHED RULE NUMBER "
              << i << " -> " << Token::getTokenTypeByText(longRules[i][0])
              << " " << Token::getTokenTypeByText(longRules[i][1]) << " "
              << Token::getTokenTypeByText(longRules[i][2]) << std::endl);
    ...
    for (size_t i = 0; i < numberOfShortRules; ++i) {
      if (shortRuleIndex[i]) {
        DEBUG("MATCHED RULE NUMBER "
              << i << " -> " << Token::getTokenTypeByText(shortRules[i][0])
...
int Syntax::ParseExpression() {
  ...
  do {
    ...
    std::reference_wrapper<Token> topToken =
        this->getFirstTerminalFromTop(tokenStack);
    ...
    DEBUG("Switch:");
    DEBUG("Row: " << topToken.get().getTokenType() << " "
                  << "Column:" << inputToken.get().getTokenType());

    switch (Syntax::precedence_table[topToken.get().getTokenType()]
                                    [inputToken.get().getTokenType()]) {
      case R:
        DEBUG("R - Branch");
        if (this->reduceStack(tokenStack, inputToken)) {
          inputToken.get().setTokenType(
              inputToken.get().getOriginalTokenType());
    ...
    if (inputToken.get().getTokenType() == Token::P_IMPLICIT &&
        topToken.get().getTokenType() == Token::P_IMPLICIT) {
      inputToken.get().setTokenType(inputToken.get().getOriginalTokenType());
      run = false;
    }
...
int Syntax::parseSyntax(int nonTerminal, int inGrammarRule) {
  ...
  static std::vector<std::vector<std::pair<SymbolTableItem::Type, std::string>>>
      calledFuncArgs;
  ...
  if (nonTerminal > START) {
    DEBUG("==================================");
    DEBUG("going through nonTerminal: "
          << nonTerminalsMap[static_cast<nonTerminals>(nonTerminal)]
          << " Token: " << actualToken.get().getText());
    DEBUG("==================================");
  } else {
    DEBUG("==================================");
  ...
  if (_semanticsCheck) {
    DEBUG(
        "_________________________________________________________SYMBOL_"
        "TABLE:");
  ...
  switch (nonTerminal) {
    ...
    case INLINE_START:
      /*   INLINE_START ⇒ ε | IFSTMNT | FORSTMT | WHILESTMT | STATEMENTS |
      ...
      if (actualToken.get().getTokenType() == Token::END_TOKEN) {  // EMPTY
        ...
        // eat 3 tokens vector HAX to distinguish between FUNCDECL_INLINE and
        // inline STATEMENTS
    ...
    case TAIL:
      ...
      if (actualToken.get().getTokenType() == Token::BRACKET_CURLY_OPEN) {
        ret = parseSyntax(BODY, inGrammarRule);
          ...
          ret = parseSyntax(Token::SEMICOLON, inGrammarRule); // FIXME uncomment
      this to support func declaration rule. // TODO logic for this needs to be
      added
      }*/
      else {
        ret = SYNTAX_ERROR;
    ...
    case HEAD:
      ...
      if ((ret = parseSyntax(Token::BRACKET_ROUND_OPEN, inGrammarRule)) !=
          RET_OK) {
        break;
      }
      ...
      if (function.get() != nullptr) {
        ...
        for (std::vector<std::pair<SymbolTableItem::Type, std::string>>::
                 const_reverse_iterator rit = function->getArgs().crbegin();
             function->getArgs().crend() != rit; ++rit) {
          this->_interpret.append(Interpret::Instructions::LOAD,
                                  -3L - (long)(index--));
        }
      }
    ...
    case BODY:
      ...
      _lex.ungetToken(actualToken);
      if (inGrammarRule != FUNCDECL && inGrammarRule != WHILESTMT &&
          inGrammarRule != FORSTMT) {
        _scope.enterScope(false);
      }
      if ((ret = parseSyntax(Token::BRACKET_CURLY_OPEN, inGrammarRule)) !=
          RET_OK) {
        break;
    ...
    case IFSTMNT:
      ...
      if ((ret = parseSyntax(Token::KW_IF, inGrammarRule)) != RET_OK) {
        break;
      } else if ((ret = parseSyntax(Token::BRACKET_ROUND_OPEN,
                                    inGrammarRule)) != RET_OK) {
        break;
      } else if ((ret = ParseExpression()) != RET_OK) {
        ret = EXPRESSION_ERROR;
        break;
      } else if ((ret = parseSyntax(Token::BRACKET_ROUND_CLOSE,
                                    inGrammarRule)) != RET_OK) {
        break;
      } else {
    ...
    case WHILESTMT:
      ...
      if ((ret = parseSyntax(Token::BRACKET_ROUND_CLOSE, inGrammarRule)) !=
          RET_OK) {
        break;
      }
      ...
      if (ret != RET_OK) {
        ...
        for (auto& b : _scope.getScope()->getBreaks()) {
          this->_interpret.getInstructionBuffer()[b]._rec._value = ip;
        }
    ...
    case FORSTMT:
      ...
      if ((ret = parseSyntax(Token::KW_FOR, inGrammarRule)) != RET_OK) {
        ...
        break;
      } else if ((ret = parseSyntax(Token::SEMICOLON, inGrammarRule)) !=
                 RET_OK) {
        break;
      } else if (ParseExpression() != RET_OK) {
        ret = EXPRESSION_ERROR;
    ...
    case RETURNSTMT:
      ...
      if ((ret = parseSyntax(Token::KW_RETURN, inGrammarRule)) != RET_OK) {
        break;
      } else if (!_semanticsCheck ||
                 static_cast<Token::tokenType>(
                     _scope.getItemByName(actualScope).get()->getType()) !=
                     Token::KW_VOID) {
    ...
    case ASSIGNMENT:
      ...
      if ((ret = parseSyntax(NAME, ASSIGNMENT)) != RET_OK) {
        break;
      } else if ((ret = parseSyntax(Token::ASSIGNMENT, inGrammarRule)) !=
                 RET_OK) {
        break;
      } else if (ParseExpression() != RET_OK) {
        ret = EXPRESSION_ERROR;
    ...
    case TYPE:
      ...
      if (!(actualToken.get().getTokenType() >= Token::KW_INT &&
            actualToken.get().getTokenType() <= Token::KW_FLOAT) &&
          !(inGrammarRule == FUNCDECL &&
            actualToken.get().getTokenType() == Token::KW_VOID)) {
        DEBUG(actualToken.get().getText());
        ret = SYNTAX_ERROR;
      } else {
        dataType = static_cast<SymbolTableItem::Type>(
            actualToken.get().getTokenType());
      }
      break;
    ...
    case FUNCTIONCALL:
      ...
      if ((ret = parseSyntax(Token::BRACKET_ROUND_OPEN, inGrammarRule)) !=
        ...
        if (_semanticsCheck) {
          auto calledFunc = _scope.getItemByName(
              calledFunctionName.back());  // get function from symbol table
          DEBUG(
              "________________________________________________________________"
              "____SEMANTICS");
          DEBUG("Checking function call:" << calledFunctionName.back());
          if (calledFunc.get() == nullptr || !calledFunc->isFunc()) {
            DEBUG("SEMANTICS ERROR: Function '" << calledFunctionName.back()
                                                << "' is not defined");
            ret = SEMANTICS_ERROR;
            break;
          }
        ...
        if ((ret = parseSyntax(PARAMETERS, PARAMETERS)) != RET_OK) {
          ...
          if (_semanticsCheck) {
            ...
            if (calledFunc.get() != nullptr) {
              ...
              if (calledFunc->getType() == SymbolTableItem::Type::VOID)
                inst = Interpret::Instructions::POP;
      ...
      if (actualToken.get().getTokenType() == Token::BRACKET_ROUND_CLOSE) {
        ...
        if (_semanticsCheck) {
          DEBUG(
              ...
              "____SEMANTICS");
          DEBUG(
              "Checking Parameters of function:" << calledFunctionName.back());
          // DEBUG("SUPERDEBUG " << calledFunctionName.back() << " SIZEIS " <<
          // calledFuncArgs.back().size() ); DEBUG("SUPERDEBUG " << "Expr" << "
          // RETIS " << ret );
          if (calledFuncArgs.back().size() != 0 && ret != RET_OK) {
            DEBUG("SEMANTICS ERROR: Parameter '"
                  << actualToken.get().getText()
                  << "' does not match the called function's positional "
                     "argument data type");
            ret = SEMANTICS_ERROR;
            break;
          } else {
        ...
        if ((ret = parseSyntax(Token::COMMA, inGrammarRule)) != RET_OK) {
          ...
          if (_semanticsCheck) {
            DEBUG(
                ...
                "______SEMANTICS");
            DEBUG("Checking Parameters of function:"
                  << calledFunctionName.back());
            if (calledFuncArgs.back().size() != 0 &&
                ret != RET_OK) {  // FIXME check the type of expression
              ret = SEMANTICS_ERROR;
              break;
            } else {
              paramCnt.back()++;
    ...
    case NAME:  // terminal IDENTIFIER
      if (actualToken.get().getTokenType() != Token::IDENTIFIER &&
        ...
        if (_semanticsCheck &&
            actualToken.get().getTokenType() == Token::IDENTIFIER) {  // FIXME
          switch (inGrammarRule) {
            case ARGUMENTS: {
              DEBUG(
                  ...
                  "________SEMANTICS");
              DEBUG(
                  "Defining function argument:" << actualToken.get().getText());
              auto actualFunction = _scope.getItemByName(actualScope);
              if (actualFunction.get() == nullptr) {
                DEBUG("SEMANTICS ERROR: Function '"
                      << actualToken.get().getText() << "' does not exist");
                ret = SEMANTICS_ERROR;
              } else {
            ...
            case VARIABLEDEF: {
              ...
              variable = _scope.define(actualToken.get().getText(),
                                       SymbolTableItem::Kind::VARIABLE);
              if (variable.get() == nullptr) {
                DEBUG("SEMANTICS ERROR: Function '"
                      << actualToken.get().getText()
                      << "' already exists in this scope, can not define "
            ...
            case FUNCDECL: {
              ...
              function = _scope.define(actualToken.get().getText(),
                                       SymbolTableItem::Kind::FUNCTION);
              if (function.get() == nullptr) {
                DEBUG("SEMANTICS ERROR: Name '"
                      << actualToken.get().getText()
                      << "' already exists in this scope, can not define "
            ...
            case OPERATION: {
              ...
              if (!_scope.isDefined(actualToken.get().getText(),
                                    SymbolTableItem::Kind::VARIABLE) &&
                  !_scope.isDefined(actualToken.get().getText(),
                                    SymbolTableItem::Kind::FUNCTION)) {
                DEBUG("SEMANTICS ERROR: Name '" << actualToken.get().getText()
                                                << "' is not defined");
                ret = SEMANTICS_ERROR;
              } else {
                DEBUG(
            ...
            default: {
              DEBUG("Something bad happened in "
                    << nonTerminalsMap[static_cast<nonTerminals>(inGrammarRule)]
                    << " rule with "
                    << nonTerminalsMap[static_cast<nonTerminals>(nonTerminal)]
  ...
  if (nonTerminal > START) {
    DEBUG("==================================");
    DEBUG("going out nonTerminal: "
          << nonTerminalsMap[static_cast<nonTerminals>(nonTerminal)]
          << " Token: " << actualToken.get().getText() << " RC: " << ret);
    DEBUG("==================================");
  } else {
    DEBUG("==================================");

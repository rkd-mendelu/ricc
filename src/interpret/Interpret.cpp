...
namespace TPJparser {
    namespace Interpret {
        ...
        Interpret::Interpret()
          :  _ip(0),
             _bp(0),
             _inst({Instruction(NOP, StackRecord(SymbolTableItem::Type::INT, Operand(0L)))}) {}

        long Interpret::getIP() {
            return this->_inst.size();
        }

        void Interpret::append(Instructions inst) {
            this->_inst.push_back(Instruction(inst, StackRecord(SymbolTableItem::Type::VOID, Operand(std::string("void-string")))));
        }
        ...
        void Interpret::saveBP() {
            DEBUG("");
            this->_stack.push_back(StackRecord(SymbolTableItem::Type::INT, (long)this->_bp));
        }
        ...
        void Interpret::dupTop() {
            this->append(Instructions::DUP);
        }
        ...
        void Interpret::printCode() const {
            ...
            for (const auto& i: this->_inst) {
                std::cout << cnt << " " << InstructionsMap.at(i._type) << " " << SymbolTableItem::TypeMap.at(i._rec._type) << " ";

                if(i._rec._type == SymbolTableItem::Type::INT) std::cout << nonstd::get<long>(i._rec._value) <<  std::endl;
                if(i._rec._type == SymbolTableItem::Type::FLOAT) std::cout << nonstd::get<double>(i._rec._value) <<  std::endl;
                if(i._rec._type == SymbolTableItem::Type::STRING) std::cout << nonstd::get<std::string>(i._rec._value) <<  std::endl;
                if(i._rec._type == SymbolTableItem::Type::BOOL) std::cout << nonstd::get<bool>(i._rec._value) <<  std::endl;
                if(i._rec._type == SymbolTableItem::Type::VOID) std::cout << nonstd::get<std::string>(i._rec._value) <<  std::endl;
        ...
        int Interpret::execute(Instruction& i) {
            ...
            DEBUG(std::endl << std::endl << "IP: " << this->_ip << " BP: " << this->_bp << std::endl);
...
#ifdef DEBUG_MODE
            DEBUG("");
            for (myStack::const_reverse_iterator i = this->crbegin() ; i < this->crend() ; ++i)  {
                DEBUG("=====================================");
                i->print();
                DEBUG("=====================================" << std::endl);
            }

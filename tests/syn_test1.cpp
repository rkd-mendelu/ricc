...
std::map < std::string, std::pair<int, int> > tests {
    ...
    {"int main () {}", {SYNTAX_OK, 1}},
    {"int main () {} int main () {}", {SYNTAX_OK, 2}},
    {"int foo () {return a;}", {SYNTAX_OK, 3}},
    {"int bar () {a=9;}", {SYNTAX_OK, 4}},
    {"int bar () {a=(;}", {EXPRESSION_ERROR, 5}},
    //               ⬑------------┛
    {"int bar () {foo();}", {SYNTAX_OK, 6}},
    {"int bar () {foo(bar);}", {SYNTAX_OK, 7}},
    {"int bar () {foo(bar,);}", {SYNTAX_ERROR, 8}},
    //                   ⬑------------┛
    {"int bar () {foo(bar,baz);}", {SYNTAX_OK, 9}},
    {"int bar () {foo(bar,baz); return a;}", {SYNTAX_OK, 10}},
    {"int bar () {foo(bar,baz); return a}", {SYNTAX_ERROR, 11}},
    //                                  ⬑------------┛
    {"int bar () {foo(bar,9); return a}", {SYNTAX_ERROR, 12}},
    //                                ⬑------------┛
    {"int bar () {foo(bar,9); return a;}", {SYNTAX_OK, 13}},
    {"int bar () {foo(bar=baz,9); return a;}", {SYNTAX_OK, 14}},
    {"int bar () {foo(bar=,9); return a;}", {EXPRESSION_ERROR, 15}},
    //                    ⬑-----------------------┛
    {"int bar () {foo(bar=baz,boo=9); return a;}", {SYNTAX_OK, 16}},
    {"int bar () {foo(bar=baz,200=9); return a;}", {SYNTAX_OK, 17}},
    {"int main (int argc) {foo(bar9); return a;}", {SYNTAX_OK, 18}},
    {"int main (int argc, string argv) {foo(bar9); return a;}", {SYNTAX_OK, 18}},
    {"int main (int argc, string ) {foo(bar9); return a;}", {SYNTAX_ERROR, 19}},
    //                          ⬑----------------------------------┛
    {"int main (int argc ;) {foo(bar9); return a;}", {SYNTAX_ERROR, 19}},
    //                   ⬑----------------------------------┛
    {"int main (int argc) {string candy; foo(bar9); return a;}", {SYNTAX_OK, 20}},
    {"int main (int argc) {string; foo(bar9); return a;}", {SYNTAX_ERROR, 21}},
    //                           ⬑--------------------------------┛
    {"int main (int argc) {string candy, shop; foo(bar9); return a;}", {SYNTAX_OK, 22}},
    {"int main (int argc) {string candy, shop;; foo(bar9); return a;}", {SYNTAX_ERROR, 23}},
    //                                        ⬑--------------------------------┛
    {"int main (int argc) {string candy=shop; foo(bar9); return a;}", {SYNTAX_OK, 24}},
    {"int main (int argc) {string candy=shop, asdf=\"Ahoj\"; foo(bar9); return a;}", {SYNTAX_OK, 25}},
    {"int main (int argc) {string candy=shop, asdf=\"Ahoj\" return a;}", {SYNTAX_ERROR, 26}},
    //                                                     ⬑--------------------┛
    {"int david () {if(zidan){print(9);} return sufurki;}", {SYNTAX_OK, 27}},
    {"int david () {if( ){print(9);} return sufurki;}", {EXPRESSION_ERROR, 28}},
    //                 ⬑-------------------------------------------┛
    {"int david () {if(zidan){print(9)} return sufurki;}", {SYNTAX_ERROR, 29}},
    //                               ⬑----------------------------┛
    {"int david () {if(zidan){print(9);}else{return sufurki;}}", {SYNTAX_OK, 30}},
    ...
    //                                          ⬑---------------------------┛
    {"int david () {if(zidan){if(motorko){print(9);}else{}}else{return sufurki;}}", {SYNTAX_OK, 33}},
    {"int david () {while(zidan){if(motorko){print(9);}else{}}return sufurki;}", {SYNTAX_OK, 34}},
    ...
    //                                                                    ⬑----------------------------┛
    {"int zidan () {for(int zidan; zidan<motorko; motorko = zidan){print(9);}}", {SYNTAX_OK, 38}},

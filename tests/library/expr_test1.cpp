...
int main() {
  ...
  for (auto p : m) {
    ...
    std::cout << "Expected Left Token: "
              << RICC::Token::getTokenTypeByText(p.second.second) << std::endl;
    std::cout << "Got Left Token: " << nextToken.get().getTokenTypeText()
              << std::endl;

    if (rc != p.second.first ||
        p.second.second != nextToken.get().getTokenType()) {

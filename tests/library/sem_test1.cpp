...
std::map<std::string, std::pair<int, int> > tests{
    ...
    //            ⬑------------------------------------------------------┛
    //            missing func body
    //------------->{"string foo(){} int bar (int bar) {int baz;
    // foo(bar,baz);}", {SEMANTICS_ERROR, 2}},
    //                                                ⬑--------------------┛ too
    //                                                many params
    {"int foo(int a, float b){} int bar (int bag) {float baz; foo(bag, baz);}",
     {RET_OK, 3}},  // TODO void not working
    // {"int add(float a, float b){return a + b;} int bar (int bag) {float lol,
    // sad ; add(bag, sad); return lol;}", {SEMANTICS_ERROR, 4}}, // FIXME
    // should be SEMANTICS_ERROR TODO ParseExpression(expType)
    // // ⬑---------------------------------┛ missmatch int/float
